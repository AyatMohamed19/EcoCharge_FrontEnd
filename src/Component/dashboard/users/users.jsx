...
console.log(token)
    ...
    useEffect(() => {
      axios
        .get("http://localhost:5000/api/users/auth/AllUsers",{ headers: {"authorization" : `Bearer ${token}`} })
        .then((res) => {setAllUsers(res.data.data)})
    },[token]);
    let deleteOne = (_id) => {
        axios.delete(`http://localhost:5000/api/users/auth/deleteUser/${_id}`,{ headers: {"authorization" : `Bearer ${token}`} })
        .then((res) =>{
